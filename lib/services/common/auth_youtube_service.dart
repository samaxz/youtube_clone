...
class AuthYoutubeService {
  ...
  Future<RatingState> getVideoRating(
    ...
    return authState.maybeWhen(
      orElse: () => const RatingState.neither(),
      authenticated: () async {
        final queryParams = {
          'id': videoId,
          'key': key,
        ...
        if (rating == 'like') {
          return const RatingState.liked();
        } else if (rating == 'none') {
          return const RatingState.neither();
        } else {
          return const RatingState.disliked();
        }
      },
    );
  }
  ...
  Future<RatingState> likeVideo({
    ...
    Uri makeUrl(RatingState rating) {
      late final Map<String, String> queryParams;
      late final Uri url;

      if (rating == const RatingState.liked()) {
        queryParams = {
          'id': videoId,
          'rating': 'none',
        };
    ...
    final rating = await getVideoRating(videoId, authState: authState);

    return rating.maybeWhen(
      orElse: () async {
        await _dio.postUri(makeUrl(rating));
        return const RatingState.liked();
      },
      liked: () async {
        await _dio.postUri(makeUrl(rating));
  ...
  Future<RatingState> dislikeVideo({
    ...
    Uri makeUrl(RatingState rating) {
      ...
      if (rating == const RatingState.disliked()) {
        queryParams = {
          'id': videoId,
          'rating': 'none',
        };
  ...
  Future<List<Subscription>> getSubscriptions({
    ...
    final subscriptions = List.from(response.data['items'])
        .map((map) => Subscription.fromJson(map))
        .toList();

    final nextPageToken = response.data['nextPageToken'];

    // * why does iе work, shouldn't while be here instead?
    if (nextPageToken != null) {
      ...
      // * how tf can i add elements to a final list, wtf
  ...
  Future<bool> subscribed({
    ...
    return authState.maybeWhen(
      ...
      authenticated: () async {
        ...
        final containsChannelId = subscriptions.any(
  ...
  Future<bool> changeSubscription(
    ...
    return subscribed
        ? unsubscribe(channelId, authState: authState)
        : subscribe(channelId, authState: authState);
  }
